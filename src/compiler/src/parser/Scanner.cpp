...
void Scanner::Init() {
	...
	noSym = 73;
	int i;
	for (i = 36; i <= 36; ++i) start.set(i, 1);
	for (i = 65; i <= 90; ++i) start.set(i, 1);
	for (i = 95; i <= 95; ++i) start.set(i, 1);
	for (i = 97; i <= 122; ++i) start.set(i, 1);
	for (i = 48; i <= 57; ++i) start.set(i, 36);
	start.set(39, 2);
	start.set(34, 5);
	start.set(46, 70);
	start.set(44, 25);
	...
	keywords.set(L"class", 6);
	keywords.set(L"if", 7);
	keywords.set(L"else", 8);
	keywords.set(L"while", 9);
	keywords.set(L"print", 10);
	keywords.set(L"return", 11);
	keywords.set(L"void", 12);
	keywords.set(L"final", 13);
	keywords.set(L"new", 14);
	keywords.set(L"package", 15);
	keywords.set(L"fun", 28);
	keywords.set(L"native", 29);
	keywords.set(L"co", 30);
	keywords.set(L"mcode", 52);
	keywords.set(L"begin", 53);
	keywords.set(L"end", 54);
	keywords.set(L"ADD", 55);
	keywords.set(L"PSH", 56);
	keywords.set(L"POP", 57);
	keywords.set(L"CALL", 58);
	keywords.set(L"ALEN", 59);
	keywords.set(L"LDA", 60);
	keywords.set(L"STORE", 61);
	keywords.set(L"GOTO", 62);
	keywords.set(L"ICONST", 63);
	keywords.set(L"EQU", 64);
	keywords.set(L"DC", 65);
	keywords.set(L"DS", 66);
	keywords.set(L"ORG", 67);
	keywords.set(L"BEG", 68);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 2:
			if (ch <= 8 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= 31) || (ch >= L'!' && ch <= L'&') || (ch >= L'(' && ch <= L'[') || (ch >= L']' && ch <= 65535)) {AddCh(); goto case_3;}
			else {goto case_0;}
		case 3:
			case_3:
		...
		case 5:
			case_5:
			if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= L'!') || (ch >= L'#' && ch <= L'[') || (ch >= L']' && ch <= 65535)) {AddCh(); goto case_5;}
			else if (ch == L'"') {AddCh(); goto case_12;}
			else if (ch == 92) {AddCh(); goto case_6;}
			else {goto case_0;}
		case 6:
			case_6:
			if ((ch >= L'0' && ch <= L'3')) {AddCh(); goto case_37;}
			else if ((ch >= L'4' && ch <= L'7')) {AddCh(); goto case_11;}
			else if (ch == L'"' || ch == 39 || ch == 92 || ch == L'b' || ch == L'f' || ch == L'n' || ch == L'r' || ch == L't') {AddCh(); goto case_5;}
		...
		case 13:
			...
			else if (ch == L'D' || ch == L'F' || ch == L'd' || ch == L'f') {AddCh(); goto case_24;}
		...
		case 17:
			...
			else if (ch == L'E' || ch == L'e') {AddCh(); goto case_18;}
			else {t->kind = 5; break;}
		case 18:
			case_18:
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_20;}
			else if (ch == L'+' || ch == L'-') {AddCh(); goto case_19;}
			else {goto case_0;}
		case 19:
			case_19:
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_20;}
		...
		case 21:
			...
			else if (ch == L'+' || ch == L'-') {AddCh(); goto case_22;}
			else {goto case_0;}
		case 22:
			case_22:
			if ((ch >= L'0' && ch <= L'9')) {AddCh(); goto case_23;}
		...
		case 35:
			...
			if (ch == L'$' || (ch >= L'A' && ch <= L'Z') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_35;}
		...
		case 36:
			...
			else if (ch == L'E' || ch == L'e') {AddCh(); goto case_21;}
			else if (ch == L'D' || ch == L'F' || ch == L'd' || ch == L'f') {AddCh(); goto case_24;}
			else {t->kind = 2; break;}
		case 37:
			case_37:
			if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= L'!') || (ch >= L'#' && ch <= L'/') || (ch >= L'8' && ch <= L'[') || (ch >= L']' && ch <= 65535)) {AddCh(); goto case_5;}
			else if ((ch >= L'0' && ch <= L'7')) {AddCh(); goto case_38;}
			else if (ch == L'"') {AddCh(); goto case_12;}
			else if (ch == 92) {AddCh(); goto case_6;}
			else {goto case_0;}
		case 38:
		...
		case 56:
			case_56:
			if (ch == L'l') {AddCh(); goto case_57;}
			else {goto case_0;}
		case 57:
			case_57:
			if (ch == L'u') {AddCh(); goto case_58;}
			else {goto case_0;}
		case 58:
			case_58:
			if (ch == L'd') {AddCh(); goto case_59;}
			else {goto case_0;}
		case 59:
			case_59:
		...
		case 63:
			case_63:
			if (ch == L'a') {AddCh(); goto case_64;}
			else {goto case_0;}
		case 64:
			case_64:
			if (ch == L'd') {AddCh(); goto case_65;}
			else {goto case_0;}
		case 65:
			case_65:
			if (ch == L'_') {AddCh(); goto case_66;}
			else {goto case_0;}
		case 66:
			case_66:
		...
		case 74:
			...
			else if (ch == L'<') {AddCh(); goto case_52;}
		...
		case 77:
			...
			else if (ch == L'i') {AddCh(); goto case_54;}
			else if (ch == L'l') {AddCh(); goto case_62;}
			else {goto case_0;}
		case 78:
			case_78:
			recEnd = pos; recKind = 69;
			if (ch == L'l') {AddCh(); goto case_61;}
